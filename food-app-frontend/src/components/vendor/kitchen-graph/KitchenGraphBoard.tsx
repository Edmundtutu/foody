...
import { useDeleteNode, useMoveNode, useToggleNode } from '@/hooks/queries/useKitchenNodes';
import { GripVertical, Loader2, Plus, Power, RefreshCw, Search, Trash2 } from 'lucide-react';
...
const NODE_PADDING_PERCENT = 8; // keep nodes away from the canvas borders
    ...
    const handleSubmitCreateNode = (event: FormEvent<HTMLFormElement>) => {
        ...
        createNodeMutation.mutate(
            ...
            },
            {
                ...
                onError: (error) => {
                    setFormError(error.message || 'Failed to create node');
                },
            },
        );
    };
                ...
                <Card>
                    ...
                    <CardContent className="space-y-3 text-sm">
                        ...
                        <div className="flex items-center justify-between">
                            ...
                            <span className="font-semibold text-foreground">{totalEdges}</span>
                        </div>
                        <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center justify-between">
                            ...
                            <span className="font-semibold text-foreground">{visibleNodes}/{totalNodes}</span>
                        </div>
                        <div className="flex items-center justify-between">
                            <span className="text-muted-foreground">Visible Edges</span>
                            <span className="font-semibold text-foreground">{visibleEdges}/{totalEdges}</span>
                        </div>
                        <Separator />
                        {Object.entries(nodeSummary).map(([key, value]) => (
                            <div key={key} className="flex items-center justify-between text-xs">
                                <span className="text-muted-foreground">{ENTITY_LABELS[key as keyof NodeSummary]}</span>
                        ...
                        {Object.entries(ENTITY_LABELS).map(([key, label]) => (
                            <div key={key} className="flex items-center justify-between">
                                <div className="flex items-center gap-2">
                                    <span className={`inline-flex h-6 w-6 items-center justify-center rounded-md text-xs font-semibold ${ENTITY_BADGES[key as keyof NodeSummary]}`}>
                                        {label.charAt(0)}
                                    </span>
                                    <span>{label}</span>
                                </div>
                    ...
                    <CardContent className="space-y-4 text-sm">
                        ...
                        <div className="space-y-1">
                            <p className="text-[11px] text-muted-foreground">
                                Last updated {lastUpdatedAt ? formatTimestamp(lastUpdatedAt) : 'N/A'}
                            </p>
                            <Button
                                variant="outline"
                                size="sm"
                            ...
                            >
                                {isRefreshing ? (
                                    ...
                                    <>
                                        <RefreshCw className="mr-2 h-3 w-3" /> Refresh Graph
                                    </>
                                )}
                            </Button>
                        </div>
    ...
    return (
        <Card>
            <CardHeader className="pb-2">
                <div className="flex items-center justify-between">
                    <div>
                        <CardTitle className="text-base">Kitchen Graph</CardTitle>
                        <p className="text-xs text-muted-foreground">
                            Showing {totals.visibleNodes} of {totals.totalNodes} node{totals.totalNodes === 1 ? '' : 's'} and {totals.visibleEdges} of {totals.totalEdges}
                            {' '}edge{totals.totalEdges === 1 ? '' : 's'} from the backend payload.
                        </p>
                    </div>
                </div>
            </CardHeader>
            <CardContent>
                <div ref={containerRef} className="relative h-[420px] w-full overflow-hidden rounded-xl border border-dashed border-border/70 bg-gradient-to-b from-background to-muted px-2 py-2">
                    {!hasGraphData && (
                        <div className="absolute inset-0 flex flex-col items-center justify-center text-center text-sm text-muted-foreground">
                            ...
                            <p className="max-w-sm text-xs">
                                As soon as nodes and edges exist in the backend, they will be plotted here with relative positioning
                                that mirrors the stored coordinates.
                            </p>
                        </div>
                    )}
                            ...
                            {resolvedNodes.map((entry) => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        {node.category?.name && (
                                            <p className="text-xs text-muted-foreground truncate">Category: {node.category.name}</p>
                                        )}
